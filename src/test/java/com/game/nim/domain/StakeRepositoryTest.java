...
class StakeRepositoryTest {
    ...
    @ParameterizedTest(name = "setting stake to {0} works")
    @CsvSource(value = {"14", "100", "-1"})
    void test_stick_state_can_be_set_as_expected(long value) {
        // given
        StakeRepository gameStateService = new StakeRepository();
        // when
        gameStateService.set(value);
        // then
        Assertions.assertThat(gameStateService.get()).as("stick count").isEqualTo(value);
    }

    @ParameterizedTest(name = "removing {1} from amount of {0} leaves {2}")
    @CsvSource(value = {"13,1,12", "13,0,13", "13,13,0", "0,1,-1"})
